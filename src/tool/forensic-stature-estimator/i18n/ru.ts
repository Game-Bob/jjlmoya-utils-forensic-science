...
export const content: ToolLocaleContent = {
  ...
  seo: [
    ...
    {
      type: 'paragraph',
      html: 'Используйте оценку как исходный диапазон для биологической реконструкции. Зафиксируйте, какая кость измерена, входила ли лодыжка в измерение, какая референсная популяция и стандартная ошибка применены. Различия в протоколе могут изменить результат на несколько сантиметров, поэтому оценку следует сопоставлять с антропологическими признаками и другими данными идентификации. Отдельно отмечайте состояние фрагмента, сторону тела и округление. Модель показывает вероятный диапазон, а не точный рост конкретного человека; итоговое заключение требует проверки специалистом. Такой журнал исходных данных делает расчёт прозрачным для повторной проверки и помогает не выдавать статистическую оценку за индивидуальное измерение.'
    }
  ],
  faq,
  bibliography,
