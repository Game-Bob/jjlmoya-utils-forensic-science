...
export const content: ToolLocaleContent = {
  ...
  seo: [
    ...
    { type: 'paragraph', html: "显示值取决于所选时间和输入参数。时间不确定时请比较多个情景，并标明哪些信息是估计值。这样可以保留误差范围，避免把曲线当作单独证据。" },
    { type: 'paragraph', html: "透明估算应区分观察数据和模型假设，并记录时间线、吸收阶段及消除速率的不确定性。图表用于比较情景，不能单独确定责任或法律结论。请同时展示结果范围和采用的假设。" },
    {
      type: 'paragraph',
  ...
  { type: 'paragraph', html: "回溯计算时请分别记录最后饮酒时间、吸收时间假设、体重、分布系数和消除速率。结果用于比较数学情景，不是直接测量，也不能替代正式法医意见。" },],
  faq,
  bibliography,
  howTo,
  schemas: [
