...
export const content: ToolLocaleContent = {
  ...
  seo: [
    ...
    { type: 'paragraph', html: "表示値は選択した時刻と入力条件に依存します。時刻が不確かな場合は複数の条件を比較し、推定した情報を明示してください。曲線を単独の証拠として読まないことが重要です。" },
    { type: 'paragraph', html: "観測データとモデルの仮定を分けて記録し、時刻、吸収、体重、分布係数、消失速度の不確かさを示してください。グラフは条件比較の学習用であり、正式な鑑定や法的判断を単独で決めるものではありません。" },
    {
      type: 'paragraph',
  ...
  { type: 'paragraph', html: "最後に飲んだ時刻、吸収時間、体重、分布係数、消失速度などの前提を記録してください。結果は条件を比較するための数学的な推定であり、正式な鑑定や法的判断を置き換えるものではありません。" },],
  faq,
  bibliography,
  howTo,
  schemas: [
