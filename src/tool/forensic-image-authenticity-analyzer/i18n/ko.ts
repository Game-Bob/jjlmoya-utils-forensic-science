...
export const content: ToolLocaleContent = {
  ...
  seo: [
    ...
    { type: 'paragraph', html: '검사 결과는 파일의 출처와 보존 절차를 확인하는 출발점입니다. 어떤 신호가 관찰되었는지, 어떤 신호는 확인되지 않았는지, 어떤 추가 자료가 필요한지 명확히 기록하면 휴리스틱 점수를 진위에 대한 확정적인 결론으로 오해하는 일을 줄일 수 있습니다.' },
    { type: 'paragraph', html: '파일 확장자나 하나의 태그만으로 형식, 편집 이력 또는 진위를 판단할 수 없습니다. 실제 헤더 구조, 픽셀 크기, 색상 프로필, 썸네일과 소프트웨어 정보를 비교하고 취득 기록과 대조하세요. 해시값은 파일 동일성을 확인하지만 촬영 내용의 진실성을 증명하지는 않습니다. 공유 서비스의 재압축과 메타데이터 제거도 고려하고, 확인한 범위와 남은 불확실성을 기록해야 합니다.' },
    { type: 'paragraph', html: '촬영 시간이나 GPS가 없다고 해서 이미지가 조작되었다는 뜻은 아닙니다. 카메라 설정, 공유 서비스, 개인정보 보호 설정과 재저장 과정에서 태그가 삭제되거나 다시 작성될 수 있습니다. 검사에서는 파일 헤더, 픽셀 크기, 색상 프로필, 썸네일, 소프트웨어 태그와 취득 기록을 함께 비교하세요. 원본을 통제된 매체에 보관하고 분석 전 해시, 복사와 변환 과정, 분석 환경을 기록하면 다른 사람이 같은 파일을 재검사할 수 있습니다. 결과는 추가 조사의 단서로 보고 출처, 진술, 장치 기록과 검증된 감정 방법을 함께 대조해야 합니다.' },
    { type: 'paragraph', html: '검사 기록에는 취득 시간, 보관 매체, 해시값, 분석 환경, 확인한 항목, 관찰 결과와 해결되지 않은 질문을 구분해 적으세요. 누락된 메타데이터만으로 조작을 단정하지 말고 파일의 출처와 보존 상태를 함께 대조해야 합니다. 다른 검토자가 같은 파일을 다시 확인할 수 있도록 복사와 변환 과정도 기록하고, 휴리스틱 점수와 최종 감정 의견을 분리해서 설명하세요.' },
